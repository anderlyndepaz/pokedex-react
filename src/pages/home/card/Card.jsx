...
export default function Card({ card }) {
    ...
    useEffect(() => {
        const dataEspecie = async () => {
            try {
                ...
                const id = URL[6];
                const api = await axios.get(`${URL_ESPECIES}/${id}`);
                setEspeciePokemon({
    ...
    useEffect(() => {
        ...
        if (especiePokemon?.url_especies) {
            ...
            const obtenerEvoluciones = async () => {
                ...
                const speciesUrl = api?.data?.chain?.species?.url;
                const URL2 = speciesUrl.split("/");
    ...
    return (
        <div className={css.card} >
            <img className={css.img_poke} src={itemPokemon?.sprites?.other["official-artwork"]?.front_default}
                alt="pokemon" />

            <div className={`bg-${especiePokemon?.data?.color?.name} ${css.sub_card}`} >
                <strong className={css.id_card}> {pokeId} </strong>
                <strong className={css.name_card}> {itemPokemon.name} </strong>
                <h4 className={css.altura_poke}> Altura:
                    {itemPokemon.height}0 cm
                </h4>
                <h4 className={css.peso_poke}> Peso:
                    {itemPokemon.weight} kg
                </h4>
                ...
                <div className={css.div_evolucion}>
                    {Array.isArray(evoluciones) ? (
                        evoluciones.map((evo, index) => (
                            <div key={index} className={css.item_evo}>
                                <img src={evo.img} alt={`Evolución ${evo.name}`} />
                                <h6>{evo.name}</h6>
                            </div>
                        ))
                    ) : (
